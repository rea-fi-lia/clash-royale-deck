...
LEAK="$(git diff --cached --name-only --diff-filter=ACMR | grep -E '^(trophy-[^/]+/.*|decks|cardhist|card-stats|card-tags|card-potential|card-eval|card-weights|card-ids|card-model-v2|matchups|matchups-v2-|synergy|card-pair-synergy-v1|card-pair-extension-synergy-v1|card-elixir-vectors-v1|band-meta|sighist-|wincon-policy|wincon-realization-|spell-tempo-|card-context-lift-|api-tags-seen|battle-schema-sample|battle-feature-buckets|pol-battle-intel-v1|pol-matchup-intel-v1|pol-card-intel-v1|pol-ranking-probe-v1|pol-elo-intel-v1|pol-tower-troop-meta-v1|pol-spell-weakness-v1|trophy-battle-events-v1|trophy-band-card-intel-v1).*\.json$' || true)"
if [ -n "$LEAK" ]; then
