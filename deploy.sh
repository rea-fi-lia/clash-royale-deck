...
LEAK="$(git diff --cached --name-only --diff-filter=ACMR | grep -E '^(decks|cardhist|card-stats|card-tags|card-potential|card-eval|card-weights|card-ids|card-model-v2|matchups|matchups-v2-|synergy|band-meta|sighist-|wincon-policy|wincon-realization-|spell-tempo-|card-context-lift-|api-tags-seen|battle-schema-sample|battle-feature-buckets|pol-battle-intel-v1|pol-matchup-intel-v1|pol-tower-troop-meta-v1|pol-spell-weakness-v1).*\.json$' || true)"
if [ -n "$LEAK" ]; then
