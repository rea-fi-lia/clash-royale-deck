...
LEAK="$(git diff --cached --name-only --diff-filter=ACMR | grep -E '^(decks|cardhist|card-stats|card-tags|card-potential|card-eval|card-weights|card-ids|matchups|synergy|band-meta|sighist-).*\.json$' || true)"
if [ -n "$LEAK" ]; then
