...
async function main() {
  ...
  for (const card of cards) {
    try {
      ...
      const hp = s16['Hitpoints'];
      if (typeof hp === 'number') card.hp16 = hp;
      const dpsKey = Object.keys(s16).find(k => /damage per second/i.test(k));
      if (dpsKey && typeof s16[dpsKey] === 'number') card.dps16 = s16[dpsKey];
