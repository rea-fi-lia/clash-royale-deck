...
export const cardBuildLink=(name,form='n')=>'<a class="me-card-build" href="'+esc(buildDeckUrl([name],[form]))+'" aria-label="'+esc(name)+'を軸にして組む" title="軸にして組む">'+window.cardImgTag(name,form,{alt:name})+'</a>';
export const deckBuildLink=(deck,forms)=>'<a class="me-build-action" href="'+esc(buildDeckUrl(deck,forms))+'">このデッキで組む <span aria-hidden="true">↗</span></a>';
...
function scopeHTML(data){
  ...
  const cardRows=data.cards.map(r=>'<tr><td><span class="me-card-label">'+cardBuildLink(r.name)+'<span>'+esc(r.name)+'</span></span></td><td>'+r.winRate+'%</td><td>'+r.wins+' / '+r.losses+(r.draws?' / '+r.draws:'')+'</td><td>'+r.games+(r.games<10?'<small>少数</small>':'')+'</td></tr>');
