...
- **統計データ（decks.json / cardhist.json / card-*.json / matchups / sighist / synergy / band-meta）は main へ push しない。** R2設定後はCloudflare R2 privateを主保存先にし、サイトはWorker API（`/api/assist/bootstrap`, `/api/assist/context`, `/api/strategy`, `/api/meta`）経由で読む。`data` ブランチ直読みは移行/緊急fallbackだけ。
