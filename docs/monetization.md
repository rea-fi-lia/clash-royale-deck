...
| 自分のデータの保持 | **3ヶ月** | **永久** |
| タグの扱い | いつでも変更・取り消し可 | **一度登録したら固定（UIから消せない）** |
...
### 課金者のタグは固定（jo決定・2026-08-11）
- 課金した人のタグは**ユーザー自身では削除・変更できない仕様**にする。
  意図＝勝手に消えてデータが途切れる事故を防ぎ、「こちらで回し続ける」を保証する。
  解約→再開しても歴史がそのまま繋がっていることが価値の核だから
- 解約後：毎時収集は止まるが、**蓄積したデータとタグは保持**（再課金すればそのまま続きから）
- ⚠️ **法律上の逃し弁だけは残す**：UIに削除ボタンは置かないが、問い合わせ経由の
  削除依頼には応じる（日本の個人情報保護法の利用停止請求・EU圏のGDPR消去権に対応。
  「本人が求めても一切消さない」仕様は法的リスクと炎上リスクが大きい）。
